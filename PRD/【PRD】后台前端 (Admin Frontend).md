...
**色彩设计规范（蓝白配色）：**
为了提供专业、高效的大型管理系统体验，后台配色参考 Zoom、腾讯会议等成熟的 SaaS 软件，采用**蓝白配色（Blue & White）**为主基调。
- **主色调（Primary Blue）：** 沉稳且具有科技感的蓝色，用于核心按钮、激活状态、关键链接。
- **背景与卡片（Background & Card）：** 整体背景采用非常浅的灰蓝色或纯白，卡片和内容区使用纯白，以确保数据表格的极高可读性和对比度。
- **侧边栏（Sidebar）：** 侧边栏可以采用深蓝/黑蓝底色搭配白字（深色模式），或纯白底色搭配蓝/灰字（浅色模式），这里默认配置为深蓝沉稳风格。

后台需要更冷静的背景色以突显数据表格，同时必须引入强烈的状态指示色。
...
@layer base {
  :root {
    ...
    /* 左侧边界栏专属颜色 (深蓝沉稳风格，参考腾讯会议) */
    --sidebar-bg: 220 80% 10%; /* 更偏蓝的深色 */
    --sidebar-fg: 210 40% 98%;
    --sidebar-hover: 216 50% 20%;
    --sidebar-active: 210 40% 98%;
    --sidebar-active-fg: 220 80% 10%;
    ...
    /* 主题色：专业蓝 (Zoom/Tencent Meeting Style Blue) */
    --primary: 217 91% 60%; /* #2D8CFF 类似色 */
