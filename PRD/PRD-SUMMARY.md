...
UTMCSSA（多伦多大学密西沙加校区中国学生学者联谊会）官网重构项目，采用"前后端分离 + Headless CMS"架构，目标是构建一套高性能、内容 100% 可视化管理、无需改代码即可更新的现代化官网体系。

项目分为两个独立子应用，后台管理直接使用 Strapi 原生 Admin UI：
...
└──────────────┬──────────────────────────┬───────────────────┘
               │                          │
    ┌──────────▼──────────┐   ┌───────────▼────────────┐
    │   客户端前端         │   │   Strapi Admin UI       │
    │   client/           │   │   localhost:1337/admin  │
    │   Next.js + ISR     │   │   内容管理后台           │
    │   面向公众用户       │   │   面向社团管理员         │
    └──────────┬──────────┘   └───────────┬────────────┘
               │                          │
