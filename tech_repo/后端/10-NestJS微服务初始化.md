...
- `backend/microservice/src/queue/queue.module.ts`（实现）
- `backend/microservice/src/queue/email.processor.ts`（实现，Task 8 占位）
- `backend/microservice/src/queue/export.processor.ts`（实现，Task 7 占位）

**Commit message：** `feat(microservice): 初始化 NestJS 项目，实现报名接口、速率限制和队列模块`
...
3. **完整业务实现**：报名流程链路全部跑通（Controller → Guard → Service → Strapi → Queue）
4. **Processor 占位**：`email.processor.ts` / `export.processor.ts` 有结构但只打日志，Task 7/8 补全
...
**为什么用 `@nestjs/bullmq` 而不是 SOP 中写的 `@nestjs/bull`：**

SOP 的依赖列表写的是旧版 `@nestjs/bull` + `bull`，但 Austin 的所有代码注释使用 BullMQ API（`WorkerHost`、`@Processor` 装饰器、`Job<T>` 类型），这是 `@nestjs/bullmq` 的接口。
`bull`（v3）已不再维护，`bullmq`（v4/v5）是官方继任者，API 不兼容但功能更完善。
代码优先于 SOP，选 `@nestjs/bullmq`。
